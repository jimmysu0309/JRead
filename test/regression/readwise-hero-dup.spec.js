// JRead — regression spec: Readwise 匯出移除重複 hero 主圖（v0.8.124）
//
// Trigger: Jimmy 2026-06-19 theverge.com hands-on 回報——送到 Readwise Reader 後
// hero image 重複顯示。
//
// 根因：payload.imageUrl（= hero URL）被 Readwise 另 render 成 cover，body 內若殘留
// 同一張 hero img 就會重複。The Verge 用 art-direction 把 hero 渲染成兩張同圖（桌機
// `_1044qizn` + 手機 `_1044qizm`，?w= 不同、pathname 相同），只移第一張會殘留另一張。
//
// 修法（NS.findLeadingHeroImage + NS.markHeroImageForExport，結構通則、非站點/class
// 特判）：image_url 取自 findLeadingHeroImage 選到的主圖；markHeroImageForExport 標記
// body 內**所有 URL pathname 與其相同**的 img（cover 的全部重複）供 buildCleanHtml
// 移除。figcaption 不在標記範圍 → 主圖圖說保留。
//
// 本 spec 驗 NS 標記邏輯（jsdom，stub naturalWidth）。真實 Chrome 端到端（payload
// body 不含 hero、image_url 仍為 hero、caption 保留）在 Playwright harness 驗過。
...
describe('readwise — 移除重複 hero 主圖（v0.8.124）', () => {
  ...
  it('(A) 兩張 hero 變體（同 pathname、不同 ?w=）都被標記移除', () => {
    const { document, NS } = setup();
    ...
    assert.ok(document.getElementById('hero-mobile').closest('[data-jread-rw-strip="1"]'),
      '手機 hero 變體（同 pathname）也必須被標記——只移第一張會殘留它');
    assert.strictEqual(marked.length, 2, '兩張 hero 變體都標記');
  });
