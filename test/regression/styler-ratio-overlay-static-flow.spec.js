...
describe('styler — ratio / object-fit 容器後代 static flow（v0.8.94 New Yorker hero）', () => {
  // v0.8.155→v1.6.30：三條選擇器各補 :not([data-jread-embed-wrap="1"])（原
  // :not(:has(iframe))，#13 insertion invalidation 改 marker attr，標記行為
  // forcing 在 insertion-invalidation.spec）——排除 responsive 影片嵌入
  ...
  it('static-flow 規則必須覆蓋 [class*="ratio"] 後代（核心修法）', () => {
    ...
    assert.ok(
      /\[class\*="ratio" i\]:not\(\[data-jread-player="1"\]\):not\(\[data-jread-embed-wrap="1"\]\)\s*\*/.test(css),
      'CSS 必須含 [class*="ratio" i]:not([data-jread-player="1"]):not([data-jread-embed-wrap="1"]) * selector（ratio 容器內 absolute overlay 拉回 static flow，但排除 iframe 影片嵌入）'
    );
  });
  ...
  it('static-flow 規則必須覆蓋 [class*="object-fit"] 後代', () => {
    const css = getInjectedCss();
    assert.ok(
      /\[class\*="object-fit" i\]:not\(\[data-jread-player="1"\]\):not\(\[data-jread-embed-wrap="1"\]\)\s*\*/.test(css),
      'CSS 必須含 [class*="object-fit" i]:not([data-jread-player="1"]):not([data-jread-embed-wrap="1"]) * selector'
    );
  });
  ...
  it('既有 [class*="placeholder"] 後代 static-flow 不退步', () => {
    const css = getInjectedCss();
    assert.ok(
      /\[class\*="placeholder" i\]:not\(\[data-jread-player="1"\]\):not\(\[data-jread-embed-wrap="1"\]\)\s*\*/.test(css),
