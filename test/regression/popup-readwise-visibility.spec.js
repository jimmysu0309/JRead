...
//
// 修法：HTML 初始 `hidden` 屬性 + popup.js refreshReadwiseButton 根據 reader
...
describe('popup v0.7.130 — 送到 Readwise Reader 按鈕可見性', () => {
  ...
  it('popup.js refreshReadwiseButton 必須對 readwiseBtn.hidden 賦值（不能只動 disabled）', () => {
    // 抓 refreshReadwiseButton function body
    const m = POPUP_JS.match(/async\s+function\s+refreshReadwiseButton\s*\(\s*\)\s*\{([\s\S]*?)\n\}/);
    assert.ok(m, '能在 popup.js 找到 async function refreshReadwiseButton');
    const body = m[1];
    // 至少要有一處 readwiseBtn.hidden = ... 賦值
    assert.match(body, /readwiseBtn\.hidden\s*=/,
      'refreshReadwiseButton 必須對 readwiseBtn.hidden 賦值——forcing：只改 disabled 會回退到 v0.7.129 之前的「按鈕灰色露出」行為');
    ...
    assert.ok(hiddenAssignmentCount >= 2,
      `refreshReadwiseButton 至少應有 2 處 readwiseBtn.hidden 賦值（早期 return + 主路徑），實測 ${hiddenAssignmentCount}——forcing：漏掉早期 return 路徑會讓「無 tab / 無 content script」情境按鈕仍露出`);
  });
});
