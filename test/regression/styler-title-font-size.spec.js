...
//
// 本 spec 驗：
//   1. titleFontSize > 0 → CSS 注入 h1 font-size override
...
describe('styler — titleFontSize（v0.7.175）', () => {
  ...
  it('titleFontSize=72 → CSS 注入 h1 { font-size: 72px }', () => {
    ...
    assert.ok(/h1\s*\{[^}]*font-size:\s*72px\s*!important/.test(css),
      'CSS 必須含 h1 { font-size: 72px !important }');
  });
  ...
  it('titleFontSize=0 (Auto) → CSS 不含 h1 font-size rule', () => {
    ...
    const css = window.document.getElementById('__jread-style').textContent;
    assert.ok(!/\bh1\s*\{[^}]*font-size:/.test(css),
  ...
  it('titleFontSize=1e308 → clamp 到 200px 以下', () => {
    ...
    const css = window.document.getElementById('__jread-style').textContent;
    const match = css.match(/h1\s*\{[^}]*font-size:\s*(\d+)px/);
    assert.ok(match, 'h1 font-size rule 必須存在');
    assert.ok(Number(match[1]) <= 200, `h1 font-size (${match[1]}px) 必須 <= 200px`);
  });
  ...
  it('titleFontSize=2 → clamp 到 8px 下限', () => {
    ...
    const match = css.match(/h1\s*\{[^}]*font-size:\s*(\d+)px/);
    assert.ok(match, 'h1 font-size rule 必須存在');
    assert.ok(Number(match[1]) >= 8, `h1 font-size (${match[1]}px) 必須 >= 8px`);
  });
});
