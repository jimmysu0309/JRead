...
// browser.* API：
//   - browser.management（JREAD_RELOAD debug bridge 用 getSelf 查 installType）
//   - browser.runtime.reload
//   - browser.commands（popup 快速鍵提示）
//   - browser.action 子集可能缺 setIcon / badge 系列
//   - browser.windows（YouTube Borderless RESIZE_OWN_WINDOW）
//
...
describe('iOS Safari API availability guards（v0.7.217）', () => {
  it('SW JREAD_RELOAD：browser.management.getSelf 呼叫前必須有 existence guard', () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (guardLine === -1 && /browser\.management\s*&&\s*browser\.management\.getSelf/.test(lines[i])) guardLine = i;
      if (callLine === -1 && /runIfDevelopmentInstall\s*\(/.test(lines[i])) callLine = i;
    }
    assert.notStrictEqual(guardLine, -1,
      'JREAD_RELOAD 必須含 `browser.management && browser.management.getSelf` existence guard（iOS 無 management API）');
    ...
    // v0.8.164：getSelf 改 Promise 形式 `getSelf().then(...)`（無 callback 參數）
    assert.ok(helper && /browser\.management\.getSelf\s*\(\s*\)/.test(helper[0]),
      'runIfDevelopmentInstall 必須實際呼叫 browser.management.getSelf()');
  });

  it('SW JREAD_RELOAD：guard 必須同時檢查 browser.runtime.reload 存在', () => {
    const body = caseBody(SW_SRC, 'JREAD_RELOAD');
    assert.ok(/!browser\.runtime\.reload/.test(body),
      'JREAD_RELOAD guard 必須含 `!browser.runtime.reload` 檢查（iOS 無 runtime.reload）');
  });

  it('SW SET_ACTIVE_ICON：browser.action 呼叫前必須有 existence guard', () => {
    const body = caseBody(SW_SRC, 'SET_ACTIVE_ICON');
    const lines = body.split('\n').filter(l => !/^\s*(\/\/|\*)/.test(l));
    let guardLine = -1;
    let callLine = -1;
    for (let i = 0; i < lines.length; i++) {
      if (guardLine === -1 && /!browser\.action\s*\|\|/.test(lines[i])) guardLine = i;
      if (callLine === -1 && /browser\.action\.setIcon\s*\(/.test(lines[i])) callLine = i;
    }
    assert.notStrictEqual(guardLine, -1,
      'SET_ACTIVE_ICON 必須含 `!browser.action || ...` existence guard（iOS action API 子集可能缺）');
    assert.notStrictEqual(callLine, -1, 'SET_ACTIVE_ICON 必須仍有 setIcon 實際呼叫');
    assert.ok(guardLine < callLine, 'guard 必須在 setIcon 呼叫之前');
  });

  it('SW RESIZE_OWN_WINDOW：browser.windows.update 必須包在 try block 內', () => {
    const body = caseBody(SW_SRC, 'RESIZE_OWN_WINDOW');
    // windows.update invocation 之前（同 case body 內）必須出現 try {
    const updateIdx = body.search(/browser\.windows\.update\s*\(/);
    ...
    assert.ok(/try\s*\{[^}]*$/.test(beforeUpdate),
      'browser.windows.update 必須在 try { 內（iOS 無 windows API，TypeError 需被吃掉）');
  });

  it('popup.js：browser.commands.getAll 呼叫必須在 existence guard 內', () => {
    const lines = POPUP_SRC.split('\n').filter(l => !/^\s*(\/\/|\*)/.test(l));
    let guardLine = -1;
    let callLine = -1;
    for (let i = 0; i < lines.length; i++) {
      if (guardLine === -1 && /browser\.commands\s*&&\s*browser\.commands\.getAll/.test(lines[i])) guardLine = i;
      if (callLine === -1 && /browser\.commands\.getAll\s*\(\s*\)/.test(lines[i])) callLine = i;
    }
    assert.notStrictEqual(guardLine, -1,
      'popup.js 必須含 `browser.commands && browser.commands.getAll` guard（iOS 無 commands API，top-level TypeError 會炸掉整個 popup）');
