...
<!-- [2026-06-26] 翻頁模式退出捲回閱讀位置：v1.6.8 已實作結案——captureExitAnchor
  fragment coverage 對映（text node Range.getClientRects 在 scrollLeft=0 下量，
  與頁數計算同款量法）。舊三法失準根因 = element bounding rect as-if-unfragmented
  聯集 + scrolled 狀態量測 + ratio 線性假設。regression：paged-mode.spec.js +
  exit-scroll-sync.spec.js；Chromium probe 三站 + e2e + iOS simulator 驗證全過。 -->
