...
6. **OVERFLOW AUDIT**（v0.8.39）：水平溢出檢查（元素衝出 card 右緣 > 2px / 整頁長水平 scrollbar），`--paged` 跳過（multicol 天生超出 viewport）。

7. **TRANSLATE-FIRST AUDIT**（v0.8.12）：`--translate-first` flag 驗收「先 Shinkansen 翻譯 → 再進閱讀模式」（對應 Jimmy macOS Safari 實機順序）。隱含 `--shinkansen`（載入 Shinkansen extension）；toggle **前**翻譯，讓主 RESIDUAL / CONTRAST / GAP audit 全跑在「翻譯後偵測的 DOM」上。**動機**：detector / cleaner 的輸出受頁面文字內容影響（Readability 評分、og:title 文字比對），翻譯把文字換成中文後會走進「英文未翻譯」從沒測過的 code path（chinatalk.media 實案：detector 把站名 logo H1 當 hero 上浮、留言+推薦括進主文）。**判別**：「只在 Safari」的雜訊先用 `--translate-first` 在 Chromium 試重現——能重現 = translation/usage-path bug（非 WebKit 引擎差異）、可在 Chromium 修 + jsdom 測。普通 `--shinkansen` 是「toggle→翻譯」、主 audit 跑在英文 DOM 上、抓不到此類。**audit 誤報注意**：翻譯後內文常含「透過 / 更多 / 追蹤」等常用詞，被 keyword audit 當子字串命中印 ⚠️——看 ancestors 在 `body.markup` / `available-content`（主文內文）即誤報，非真雜訊。

**audit 邏輯的單一資料源在 `tools/audit-lib.js`**（v0.8.39）：NOISE_AUDIT_KEYWORDS 與全部 audit 函式由 debug-harness 與 page-rounds-harness 共用，改 audit 規則只動 lib、不在 harness 內 inline copy（anti-drift forcing function：`test/regression/harness-audit-lib.spec.js`）。page-rounds 的 set-theme 經 `setThemeAndVerify` 驗 card bg 真的變色才截 dark 圖（v0.8.36 SW gate 後 dispatch 不保證生效，沒驗就是偽陰性）。
