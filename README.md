...
- 懸浮控制 icon（v0.8.154）：頁面邊緣常駐一顆小圖示——**短按**切換閱讀模式、**長按**開選單（送到 Readwise Reader、切換分頁模式）、**拖曳**可換到左 / 右緣與上下位置。Safari（iPhone / iPad / Mac）預設開、桌面瀏覽器預設關，可在進階設定的「觸控與懸浮控制」開關、調整透明度與尺寸（小 16px / 大 32px，覺得太小不好點可放大，v0.8.156）
- 三指輕點切換閱讀模式可關（v0.8.154）：觸控裝置三指輕點切換閱讀模式（iPhone / iPad 主要入口）可在進階設定開關，v0.8.157 起預設關閉（易誤觸、懸浮 icon 已是觸控主入口）
- 回復預設設定（v0.8.157）：options 頁最下方一顆「回復預設」按鈕，一鍵把所有設定還原成預設值（需點兩次確認避免誤觸），但會保留 Readwise / Gemini 的 API key
