...
- 進入 Reader（v1.0.22）：popup 按「進入 Reader」開新分頁，直接列出 Readwise 收件匣最新十篇文章、跳過 Readwise 網站。每篇可一鍵「封存」；點進文章用 JRead 原本的閱讀版型呈現，主題 / 字型 / 字級 / 翻頁全部沿用原本的控制（不用另學新 UI），也會記住閱讀位置（需在進階設定填入 Readwise token）。也可在一般網頁長按懸浮按鈕從選單「進入 Reader」（v1.0.23）
