...
- 懸浮控制 icon（v0.8.154）：頁面邊緣常駐一顆小圖示——**短按**切換閱讀模式、**長按**開選單（送到 Readwise Reader、切換分頁模式）、**拖曳**可換到左 / 右緣與上下位置。Safari（iPhone / iPad / Mac）預設開、桌面瀏覽器預設關，可在進階設定的「觸控與懸浮控制」開關與調整透明度
