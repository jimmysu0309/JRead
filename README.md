...
- 懸浮按鈕（v0.8.154）：頁面邊緣常駐一顆小圖示——**短按**切換閱讀模式、**長按**開選單（送到 Readwise Reader、切換分頁模式）、**拖曳**可換到左 / 右緣與上下位置（預設在左下角，v0.8.160）。觸控裝置會自動讓按鈕停在離上下角落安全距離的位置，避免擋到 iPad 視窗縮放把手而拖不出來，關掉再開也會回到預設位置（v0.8.161）。全平台預設開（v0.8.158），可在進階設定的「觸控與懸浮控制」開關、調整透明度與尺寸（小 16px / 大 32px，覺得太小不好點可放大，v0.8.156）
