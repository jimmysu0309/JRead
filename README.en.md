...
- **Two reading modes (pick one)**: the default is **scroll mode** (ordinary vertical scrolling); it can be switched to **paged mode** in settings or with the ⌥P shortcut (pressing ⌥P outside reading mode enters it directly in paged mode) — flip left/right like an e-book, swiping on phones or using arrow keys / the scroll wheel on desktop, with images scaled to fit a single page. The page-number footer doubles as a scrubber: hold and drag to jump through pages, with a progress bar and per-page haptic feedback while dragging
...
- **Custom shortcuts**: record key combinations for toggle reading mode / send to read-later service / YouTube borderless mode / toggle paged mode; Safari (including an iPad external keyboard) has no browser-level rebinding entry, so this is the only path there, and it works on Chrome too
