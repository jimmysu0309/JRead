...
最新：**v1.5.19**。詳細修法見 [`CHANGELOG.md`](CHANGELOG.md) 頂部條目；`package.json` / `jread/manifest.json` 為真實版本號來源（`test/version-check.spec.js` forcing function 強制四邊同步：manifest / package.json / SPEC / CHANGELOG）。
...
│   │   ├── main.js              # 進入點、事件串接
│   │   └── orion-detect.js      # Orion（Kagi）瀏覽器偵測：同檔掛兩個 content_scripts entry（隔離世界 + world:MAIN，皆 document_start），三法輪試讀 window.kagi（direct / wrappedJSObject / 注入 script）→ 替 <html> 蓋 .jread-orion + 設 --jread-orion-top（v1.5.18，v1.5.19 改隔離三法）
│   ├── popup/
│   │   ├── popup.html
│   │   └── popup.js
│   ├── options/
