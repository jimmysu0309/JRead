...
最新：**v1.7.48**。詳細修法見 [`CHANGELOG.md`](CHANGELOG.md) 頂部條目；`package.json` / `jread/manifest.json` 為真實版本號來源（`test/version-check.spec.js` forcing function 強制四邊同步：manifest / package.json / SPEC / CHANGELOG）。

### 品質底線與硬規則（跨版本長期適用）

「不可退讓底線」由測試與流程動態承載，不再維護版本錨點快照：每次 release 後，`test/regression/`（每條實機 bug 的 forcing function fixture）+ harness `RESIDUAL AUDIT` + `/release` 流程即是新的底線。歷史錨點（v0.6.3 / v0.7.32 / v0.7.135）與各版功能快照見 `CHANGELOG.md`；各功能的現行規格見本檔後續章節。

**本檔獨有的硬規則**（其餘操作性硬規則單一資料源在 skills 與 `CLAUDE.md`，本檔不重抄）：

1. **styler 瘦身不變式不可破**（v0.6.0 起的產品識別）：字型 / heading margin / p margin / list style / link color / blockquote border 全部保留原站樣式；styler 只注入讀者卡片容器 + 祖先鏈 reset + 必要 hack + 使用者 override；禁止恢復 v0.5.x 對 h1-h6 / p / ul / ol / li / blockquote / a 下 rule 的做法。forcing function：`test/regression/styler.spec.js`（v0.6.0 瘦身版）。修 bug 可動 styler 但為最後手段；**主動性 typography 重設計（非 bug 修法）需 Jimmy 明確授權**。
2. **修法優先順序**：detector → cleaner → styler（最後手段）。
3. **Typography-affecting universal rule 必須用 scoped selector**（硬教訓 20，v0.7.17 → v0.7.18 theverge 事故）——`width` / `max-width` / `margin` / `padding` 等影響版面的幾何屬性若寫 `* { ... !important }` 會破 drop cap / figure full-bleed / styled-components 既有寬度；universal rule 只適合副作用不影響 layout 的屬性（如 `background-color: transparent`）。改 styler 後**必須** harness 截圖 + Read 自驗整頁排版，不能只看 residual audit。

**指向單一資料源的硬規則**：

- 所有 interactive button 一律清、`hide()` 用 inline `!important`、delayed lazy-inject 靠邏輯完整性（MutationObserver subtree + 遞迴 check + inline `!important` 齊備）→ `/new-noise-rule` skill
- 修 detector / cleaner / styler 類 DOM 互動 bug 先驗假設再動 code → `/probe` skill + `CLAUDE.md`「假設驗證順序」
- 偽陰性驗收禁止（`RESIDUAL AUDIT` 為基準；「Jimmy 看到 / harness 看不到」信 Jimmy 截圖）→ `/harness-verify` skill + `CLAUDE.md`「Harness 驗收」
...
│   │   ├── settings-defaults.js # DEFAULT_SETTINGS 單一資料源（content / SW / Safari、Firefox event page 共用，v0.7.235；另含 FONT_STACKS / LATIN_FONTS / PROGRESS_BAR_STYLES / 憑證裁剪）
│   │   ├── domain-match.js      # 萬用字元網域比對（content / popup / options / spec 共用）
│   │   ├── link-follow.js       # 點文內連結時目標頁自動延續閱讀模式的純邏輯模組（v1.6.14）
...
│   │   ├── floating-icon.js     # 頁面邊緣懸浮按鈕：短按 toggle / 長按選單 / 拖曳吸邊（v0.8.154）
...
│   │   ├── styler.js            # 套用乾淨排版（apply 33 個具名 pass + buildCss 10 段 base segment，v1.7.48 重構）
│   │   ├── space-scroll.js      # Space 段落焦點卷動 + 指示條（v0.7.216）
│   │   ├── paged-mode.js        # 翻頁模式：手勢/鍵盤/滾輪翻頁 + 頁碼指示 / scrubber（v0.7.227）
│   │   ├── position-memory.js   # 閱讀位置記憶：段落/頁碼持久化 + 回復（v0.8.40）
│   │   └── main.js              # 進入點、事件串接、列印防護（v1.5.27 top frame 注入 @media print 隱藏所有 JRead 注入 UI）
│   ├── popup/
│   │   ├── popup.html
│   │   ├── popup.js
│   │   └── popup-core.js        # 服務無關 dispatcher + 送出 / Reader API（popup / options / reader 頁 + SW importScripts 共用，v1.6.0）
│   ├── options/
│   │   ├── options.html
│   │   └── options.js
│   ├── reader/                  # 擴充自有的 JReader 頁（v1.0.22）
│   │   ├── reader.html          # 文章列表（Article Feed）
│   │   ├── reader-feed.js
│   │   ├── article.html         # 單篇（Article View，載完整 reader 堆疊、不載 detector / cleaner）
│   │   └── reader-article.js
│   ├── lib/
│   │   ├── instapaper.js        # Instapaper Full API client（OAuth 1.0a + xAuth，v1.6.0）
│   │   └── instapaper-keys.js   # consumer key（**gitignored**，含 iOS Resources 鏡像；缺檔時送出回報 CONFIG）
│   ├── assets/
│   │   ├── fonts/               # 內嵌 woff2：Noto Serif TC 三字重 + 4 支拉丁可變字型（授權見 LICENSE.txt）
│   │   └── icons/               # action icon（一般 / disabled 各 4 尺寸）
│   └── LICENSE
│                                # （站點特判隔離區 site-overrides/ 為規劃保留位置，目前無任何站點特判、目錄未建立）
├── test/                        # **不在本 repo**（2026-08-06 起移出，見下方註記）
│   ├── version-check.spec.js    # 版本號 forcing function
│   ├── helpers.js
│   ├── regression/              # jsdom 回歸測試（393 支 spec）
│   │   ├── fixtures/
│   │   └── *.spec.js
│   ├── e2e/                     # 真實 Chromium e2e（另跑 npm run test:e2e，不含在 npm test）
│   └── PENDING_REGRESSION.md
├── tools/
│   ├── audit-lib.js             # harness audit 共用 library（NOISE_KEYWORD_TIERS 兩級 keyword + 全部 audit 函式的單一資料源，v0.8.39）
│   ├── debug-harness.js         # Playwright 自動化除錯 harness（單站深驗；flag 決策樹見 /harness-verify skill）
│   ├── page-rounds-harness.js   # 批次視覺驗收（5 組分頁截圖 + audit.json + 四態 verdict：pass/review/failed/blocked + failReasons）
│   ├── batch-page-rounds.sh     # 批次跑站清單（解析 gitignored 的 docs/excluded/page-rounds-sites.md）
│   ├── e2e-harness.js           # e2e spec 用 SW 啟動樣板（test/e2e/ import）
│   ├── paged-img-probe.js       # 翻頁模式遠欄圖載入 probe（v1.0.25）
│   ├── reader-harness.js        # JReader 讀入端 harness（**已 stale**：仍驗 v1.5.3 移除的返回鈕，直接跑會失敗；驗 reader 讀入端改寫一次性 probe）
│   ├── firefox-build.sh         # Firefox sideload ZIP 重建（jq 改 manifest）
│   ├── generate-icons.js        # action icon 產生器
│   ├── generate-ios-appicon.js  # iOS App Icon 產生器
│   ├── export-promo-tiles.js    # 商店宣傳圖匯出
│   ├── asc-provision-ios.js     # iOS 簽章資源 bootstrap（憑證 / bundle ID / profiles，idempotent）
│   └── asc-expire-builds.js     # ASC 舊 build 過期處理
...
│   ├── ios-build.sh             # sync Resources + bump pbxproj + archive + export .ipa + altool 上傳 TestFlight
...
├── docs/                        # 同時是 GitHub Pages 站台根目錄
│   ├── index.html               # landing page（中英 i18n；四顆下載鈕版本號由 release.sh 自動同步）
│   ├── open.html                # iOS「加入主畫面」跳板頁（?jread-open=feed|article，v1.5.12）
│   ├── CHROME_EXTENSION_DEBUG.md # 自動化除錯完整指南（可移植給其他 extension 專案）
│   ├── NOISE_KEYWORD_RESEARCH.md # cleaner keyword 對標研究與「不可採用」結論
│   ├── CHANGELOG-archive.md     # 早期版本變更紀錄
│   ├── store-assets/            # 商店素材（icon / marquee / promo）
│   └── excluded/                # **gitignored** 本機工作文件（page rounds 清單與報告、review 報告、規劃稿）
├── .claude/skills/              # Claude Code 流程 skill（release / harness-verify / probe / new-noise-rule / ios-release）
├── .agents/skills/              # 上者的 Codex 鏡像（改一邊要同步）
├── .github/workflows/
│   └── release.yml              # tag push 觸發 → build Chrome + Firefox + source ZIP
├── BUILD.md                     # AMO reviewer Firefox 重建指引
├── CLAUDE.md                    # Claude 協作規則
├── AGENTS.md                    # 同上的 Codex 版鏡像
├── SPEC.md                      # 本檔
├── README.md / README.en.md
├── CHANGELOG.md
├── package.json                 # 僅測試用（mocha / jsdom / playwright）
└── release.sh
```

**`test/` 為何不在本 repo（2026-08-06）**：回歸測試套件已移至私有 repo，clone 回 `test/` 路徑掛載（該路徑在本 repo 已 gitignore）。**路徑保持不變是刻意的**——spec 內大量 `path.join(__dirname, '..', ...)` 讀 `SPEC.md` / `jread/` / `safari-app/`（版本號校對、manifest 結構、pbxproj 打包檢查等 forcing function），掛在原位就完全不需要改路徑，`npm test` 與 `release.sh` 照跑。**不用 git submodule**：那會在公開 repo 留下指向私有 URL 的 `.gitmodules`。掛載指令見 `.gitignore` 內註解。對外部讀者的影響：本 repo 的 extension 原始碼（`jread/`）與 build 流程完整、可自行建置，只是跑不了測試套件。
...
**發佈節奏（Jimmy 2026-06-11 起：每個 release 都跟發 TestFlight）**：`safari-app/ios-build.sh` 在**技術上**與 `release.sh` 解耦——`release.sh` 只處理 Chrome / Firefox（測試 → tag → push → GitHub Actions），iOS 是接在後面獨立跑的一步，不是同一支腳本。但**流程上不再是「要發才發」**：`release.sh` 成功後一律接跑 `./safari-app/ios-build.sh` 上傳 TestFlight（單一資料源在 `/release` skill 第 5 節 + `/ios-release` skill）。版本號已在 bump 階段同步、`ios-build.sh` 直接取 manifest version，不會撞「同版本號重傳被 ASC 拒絕」。
...
驗證分三層，各層驗的訊號不同、彼此不可互相代替（`CLAUDE.md`「視覺/行為驗證」與 `/harness-verify` skill 為操作面單一資料源）。**`test/` 自 2026-08-06 起住私有 repo、不在本 repo**（掛載方式見「檔案結構」節），以下描述的是完整開發環境下的驗證分層。

**第一層 — jsdom regression（`npm test`，約 1 分鐘）**

- `test/version-check.spec.js`：forcing function，`EXPECTED_VERSION` 常數每次 bump 必須同步更新（連帶校對 manifest / package.json / SPEC / CHANGELOG 四邊）
- `test/regression/`：393 支 spec、3,696 條斷言（v1.7.48 基準）。每修一個 bug 補一個 fixture + spec（見 `CLAUDE.md` 硬規則 4）
- **驗的是**：邏輯正確性、API 結構、DOM attribute、注入 CSS 字串內容、還原可逆性、跨檔一致性不變式（雙實作 drift 校對）
- **驗不到的**：真實排版與視覺（jsdom 無 layout）、瀏覽器引擎差異、SPA / lazy-inject 時序
- `test/PENDING_REGRESSION.md`：抽不出最小重現結構時的待辦佇列（路徑 B）

**第二層 — Playwright harness（真實 Chromium）**

- `tools/debug-harness.js`（單站深驗）+ `tools/page-rounds-harness.js` / `tools/batch-page-rounds.sh`（批次視覺驗收）；audit 邏輯單一資料源 `tools/audit-lib.js`（forcing function `test/regression/harness-audit-lib.spec.js`）
- **驗的是**：RESIDUAL / GAP / CONTRAST / OVERFLOW / PAGED audit + 截圖肉眼巡（Claude 自己 Read，不請 Jimmy 貼）
- **驗不到的**：WebKit（Safari / iOS）引擎行為、被 bot detection 擋掉的站、真實觸控手勢

**第三層 — 平台專屬**

- `test/e2e/`（`npm run test:e2e`，真實 SW 啟動；**不含在 `npm test`**）
- iOS：模擬器自驗（`/ios-release` skill）+ TestFlight 實機——WebKit 排版、觸控、iOS background 生命週期只有這層驗得到
- 仍需 Jimmy 手動驗的僅三類：keyboard shortcut 對映與衝突、popup 點擊互動、使用體感
