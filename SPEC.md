...
`0.7.18`（revert v0.7.17：universal rule width/max-width 打破 theverge drop cap + figure 排版）。Jimmy 2026-04-24 實測截圖揭露 v0.7.17 的 universal `width: auto !important + max-width: 100% !important` 雖修好 p/img 對齊 4-10px 偏移，卻**同時破壞 drop cap + figure 排版**——drop cap `D` float:left 原本靠內嵌 style 的固定 width 讓文字繞排，`width: auto` 強制破壞 intrinsic width 導致 drop cap 與首段文字重疊；figure 內 img 原本 `max-width: none` 允許滿版 bleed，universal rule 的 `max-width: 100%` 打破了 full-bleed 意圖造成部分 figure 內容溢出 card 邊界。Jimmy 硬教訓 20：「你自己改完都不看一下嗎？」universal selector 是**全站掃地雷**的動詞，連 figure/figcaption 以外的 inline `<style>` 在 p/img/div/span 上都會被強制覆寫、副作用遠大於預期。通則：typography-affecting universal rule（width / max-width / margin / padding 等影響版面的幾何屬性）必須用 scoped selector（例如 `[data-jread-active] > article > p` 或 styled-components hash class attribute selector 精準命中目標）；v0.7.16 的 background transparent universal rule 沒此問題，是因 background 屬性副作用只有「變透明」不影響 layout。本版純 revert v0.7.17 的 styler.js + styler.spec.js 變更。theverge p 4-10px 偏移未修（入 PENDING_REGRESSION）。harness timeout 升級 30s→60s + fallback `domcontentloaded`（theverge 類重站 30s 不夠）留用、跟 revert 一併 commit。188 spec 全過（移除 2 條 `assert.ok` 在同一 `it()` block 裡，test case 數不變）。

`0.7.17`（theverge p 鎖寬修法：universal rule 加 width/max-width）。**本版造成 regression，已由 v0.7.18 revert**。Jimmy 2026-04-24 回報 theverge 圖片偏左、段落偏右不對齊。Harness probe：p class `duet--article--dangerously-set-cms-markup _8enl99j _1xwtict1` styled-components 設 `width: 588px` 固定值；img 608px 更寬 + left 336px 更靠左；card padding area 邊界是 336px，p 實際 left 340-346（p 比 img 多 4-10px left offset、段落看起來靠右）。v0.7.16 universal selector `*:not(...)` 只處理 background，擴 width/max-width 覆寫 styled-components 固定 px width。既有 baseline spec 禁 `] p {` 字面 pattern、universal selector 繞過（且 width/max-width 非 typography 屬性、不違反 v0.6.0 baseline 精神）。harness 五站無 regression（theverge Playwright 偶 timeout、跟本輪修法無關）。188 spec 全過。

`0.7.16`（theverge 裝飾 background 清除）。Jimmy 2026-04-24 回報 theverge.com/report/914244 排版亂七八糟（白色 inset box 在米色 card 內、綠色/紫色 accent 色塊、drop cap M 撐出 indent）。Harness probe 揭露：articleEl 內多個 styled-components 裝飾 wrapper 有非透明背景——`duet--layout--entry-body-container`（600×10871 白底，撐起主文整體白色、形成「card 中有 card」視覺）、`_1wu3rm1`（300×300 白色 inset box）、`qnnwq1`（65×22 綠色 accent bar，h2 上方標記）、`tly2fw0`（600×92 淺紫 newsletter/引述 block）。v0.7.10 `collapseInnerGridFlex` 只處理 grid、這些都是純 `display: block` + background-color。通則修法：styler CSS 加 `*:not(figure):not(figcaption):not(summary):not(blockquote):not(code):not(pre):not(table):not(thead):not(tbody):not(tr):not(th):not(td):not(mark):not(kbd)` universal selector 強制 `background-color: transparent !important` + `background-image: none !important`。preserve 清單涵蓋 W3C 語意保留（figure/figcaption/summary/blockquote）+ 視覺慣例需要背景區隔的 tag（code/pre/table row 交替色 / mark highlight / kbd 鍵盤按鍵白底）。reader mode 精神：card 本身已有統一米色底、內部不該再有彩色裝飾打斷閱讀流。六站驗收（theverge / esmchina / ebc / line today / udn / chinatimes）全 `✅ 無殘留雜訊`。188 spec 全過。

`0.7.15`（esmchina Bootstrap col-* 鎖窄欄寬修法）。Jimmy 2026-04-24 回報 esmchina 內文寬度不對（標題撐到 card 滿版、但內文段落只占 card 寬 40%）。Harness probe 揭露：主文 `<p>` 祖先 `DIV.col-md-9.article-left` 的 computed `width: 288px`（Bootstrap `.col-md-9 { width: 75% }` 在 container inner 384px 下的值），把內文鎖在窄欄。v0.7.10 `collapseInnerGridFlex` 只處理 `display: grid` 容器、不處理 block element 的 width CSS。通則修法：styler CSS 加 Bootstrap col-xs/sm/md/lg/xl 系列 wrapper reset——`[data-jread-active] [class*="col-md-"]` 等五條 attribute selector 強制 `width: auto !important` + `max-width: none !important` + `float: none !important` + `flex: initial !important`。Bootstrap grid class 是跨 CMS 標準（WordPress / Django / Rails 普遍用）、非站點特判。attribute selector 精準命中 col-xs-* / col-sm-* / col-md-* / col-lg-* / col-xl-*；不誤殺 `.color-primary` / `.collapse` 等無 `-` 分隔的類命名。Spec 驗 CSS 含 5 條 selector + 4 條 declarations。Harness 五站驗收（esmchina / ebc / line today / udn / chinatimes）全 `✅ 無殘留雜訊`。187 spec 全過。

`0.7.14`（udn narrowPromotedSiblings 漏 h1-self guard 修法）。Jimmy 2026-04-24 回報 udn.com/news/story/124844/9460037 標題消失。Harness probe 揭露 v0.7.12 `narrowPromotedSiblings` 對「sibling 自己就是 h1」的 case 漏 guard——該 rule 原 guard `sib.querySelector('h1') continue` 檢查 sibling **後代**是否含 h1，但 `querySelector` 不含 sibling 自己、當 sibling 是 h1 element 時（如 udn 的 `<h1 class="article-content__title">` 是 `<section class="article-content__wrapper">` 的 direct child、與 `<article class="article-content">` 是兄弟），`sib.querySelector('h1')` 回 null → h1 被 hide → 標題消失。修法一行：guard 前加 `if (sib.tagName === 'H1') continue;`。Fixture `udn-h1-direct-child-narrow-guard.html` 精確重現 udn DOM（h1 + article 作為 section 的並列 children、promote 升到 section）；4 條 spec（detector 命中 + promote 升級 + h1 保留 + non-h1 sibling 仍 hide）；sanity check 註釋 H1 guard → h1 fail。harness 五站全 `✅ 無殘留雜訊`（ebc / line today / udn / chinatimes / esmchina）。186 spec 全過。

`0.7.13`（esmchina 5 層深結構支援 + partner keyword）。Jimmy 2026-04-24 回報 esmchina.com/news/14116.html 標題消失。Console probe 揭露：og:title 空、document.title 含 en dash 但前後無空格、detector getCanonicalTitle 雖 fallback 到 document.title 仍能 titleMatches，但**主文 DOM 5 層深**（container > col-md-9 > unnamed > article-cnt > article-words > article_text），`PROMOTE_MAX_HOPS=4` 不夠。v0.7.12 硬教訓 19 已預測「narrow 機制會自動兜底 → 單純放寬 PROMOTE_MAX_HOPS 通常仍 OK」，本輪直接放寬 4→5。另 esmchina `partner-content-article` sidebar 殘留 `更多>` CTA，`partner` 詞在 class 命名幾乎都指商業夥伴/廣告合作，加進 NOISE_KEYWORD_RE。Fixture `esmchina-promote-5-hops.html` 重現 5 層 single-child wrapper + partner-content sibling；5 條 spec + 字面 forcing `PROMOTE_MAX_HOPS >= 4`（既有 ebc fixture 改成 `>= 4` 比對、不鎖死具體值）；sanity check 退回 PROMOTE=4 → 2 條 esmchina spec fail。182 spec 全過、harness 五站驗（ebc / line today / udn / chinatimes / upmedia）全無新 regression。

`0.7.12`（ebc 深層 single-child wrapper + 橫向 sibling chrome 修法：promote+narrow 聯動）。v0.7.8 記入 PENDING_REGRESSION 的 ebc h1 missing 問題本輪完整修法。核心變更三處聯動：(1) detector.js `PROMOTE_MAX_HOPS` 3→4 讓 4 層深結構也能升到 common ancestor；(2) detect() 結果加 `result.promotedFrom`——promote 升級前的 el 紀錄；(3) cleaner 新 `narrowPromotedSiblings(articleEl, promotedFrom, hidden)`——從 promotedFrom 沿祖先鏈走到 articleEl，每一層 `parent.children` 中除 content 分支（contains(promotedFrom)）與 h1 分支（querySelector('h1')）外，其他 sibling chrome 全 hide（相關新聞列表 / 聽新聞 controls / share_box / article_cover 等）。main.js 把 `{ promotedFrom }` 傳給 `cleaner.clean()`。演算法與既有 `hideAncestorSiblings` 方向相反——那條從 articleEl 往 body 走、此條從 promotedFrom 往 articleEl 走，作用層不重疊互補。順手 NOISE_KEYWORD_RE 加 `controls` 詞（ebc `article_controls` 內「聽新聞」UI 殘留）。Fixture `ebc-promote-narrow-sibling-chrome.html` 模擬 4 層深 DOM + 3 層 sibling chrome；7 條 spec + 字面 forcing `PROMOTE_MAX_HOPS = 4`；sanity 註釋 narrowPromotedSiblings → 2 條 sibling hide assertion fail。harness 四站全過（ebc / line today / chinatimes / udn `✅ 無殘留雜訊`；upmedia audit false positive 與本輪無關）。177 spec 全過。

`0.7.11`（Medium click-to-zoom button wrapper 保留主文圖片）。Jimmy 2026-04-24 回報 Medium 文章圖片不見；Console probe 揭露 Medium 把主文 `<picture>/<img>` 嵌在 `<div role="button" tabindex="0">` wrapper 裡（點擊查看大圖、a11y 有「Press enter or click to view image in full size」span）。v0.7.3 `hideInsideArticleAllButtons` 「所有 button 無條件清」rule 把這個 wrapper 當純 CTA 給 hide、連帶內部 picture/img 都不可見。通則修法：`hideInsideArticleAllButtons` 加 media guard——`btn.querySelector('img, picture, video')` 命中時保留整個 wrapper。svg 不在保護範圍（svg 多為 icon、非主文媒體）。修法精神與 v0.7.8 的 h1 wrapper guard 一致：button 內含「主文載體」（h1/h2 標題、img/picture/video 媒體）時保留 wrapper，純 CTA 照清。Fixture `medium-click-to-zoom-button.html` 三條 forcing：`#zoom-btn`（role="button" 含 picture）驗保留、`#share-btn`（純 CTA）驗 hide、`#icon-btn`（svg icon）驗 hide。sanity check 註釋 guard → 2 條 assertion fail。170 spec 全過。

`0.7.10`（BBC pathological 固定 px grid container 強制 block reset）。v0.7.9 修好廣告 wrapper 後 Jimmy 回報 BBC 主文仍鎖在窄欄寬度；Console probe 揭露 articleEl **內部**多層 grid container，其中一層 `grid-template-columns: 386px` 把主文 `<p>` 鎖死。既有 `data-jread-ancestor` reset 只處理 articleEl **外部**祖先鏈、`collapseGridWithHiddenCell` 只在有 hidden child 時觸發，兩者都漏掉此 case。新函式 `collapseInnerGridFlex`：掃 articleEl 內 `display: grid|inline-grid` 且 `grid-template-columns` 含 `\d+px` 的 container，強制 `display: block !important` + `grid-template-columns: none !important`。精準條件避免誤傷 intentional 多欄 grid（`1fr 1fr` / `auto` / `minmax(0, 1fr)` 等彈性單位保留）；`flex` container 不動（由 `collapseGridWithHiddenCell` 針對 hidden child 場景處理）；`isInPreserved` 保留 figure/figcaption/summary/blockquote 內部。`restoreInnerGridFlex` 逆向還原原 display / grid-template-columns/rows。Fixture `bbc-inner-grid-fixed-column.html` 雙層 forcing：`#pathological-grid`（386px 固定）驗 reset、`#intentional-grid`（1fr 1fr 彈性）驗保留、restore round-trip 驗還原。sanity check 註釋 rule 呼叫 → 2 條 assertion fail。165 spec 全過。

`0.7.9`（BBC React styled-components 廣告 wrapper 清除）。Jimmy 2026-04-24 實測 bbc.com/news/articles/clyepyy82kxo 右側 540×1100 灰色占位，DevTools 抓 DOM 揭露結構：`<div data-testid="ad-unit" data-component="ad-slot" class="sc-66bf5539-0 euXOeE">`。class 是 styled-components hash（無 keyword 可命中）；內層 `.dotcom-ad-inner` 已被 AD_BOUNDARY_RE hide，但外層 React wrapper 靠 styled-components min-height CSS 仍撐 540×1100 空間。通則修法：`THIRD_PARTY_AD_SEL` 擴充 `[data-testid="ad-unit"]` / `[data-testid="ad-slot"]` / `[data-component="ad-slot"]` / `[data-component="ad-unit"]` 四條 React component data attribute selector。`data-testid="ad-unit"` / `data-component="ad-slot"` 是 Google Ad Manager + React 新聞站**跨站業界標準命名**（BBC / Vox / Vice / Insider 等皆用），屬結構性通則非站點特判（硬規則 3）。Fixture `third-party-ads-inside-article.html` 新增 3 條 React wrapper（id 刻意無 ad 邊界、class 是 sc-hash、只能靠 data attribute 命中）驗 3 條 selector branch；sanity check 通過（移除 4 條 selector → 3 條 assertion fail）。160 spec 全過。

`0.7.8`（ebc 媽祖廣告清除 + 主文標題 wrapper 保護 guard）。Jimmy 2026-04-24 回報 news.ebc.net.tw /news/society/548318：(A) 主文標題不見（h1 漏 scope）；(B) 文末「與媽祖同行！2026 遶境全攻略看東森」廣告殘留。Probe 確認：媽祖廣告 wrapper class 是 `.inline_text.has_marker`、class 中 `marker` 詞跨 CMS 幾乎都指「標註 / 插播 / 推薦」特殊內容（非主文），既有 NOISE_KEYWORD_RE 無 marker 詞漏網。修法 (1)：NOISE_KEYWORD_RE 擴充 `marker` 詞，通則命中 ebc `has_marker`、WordPress `post-marker`、CMS 類 `ad-marker` / `sponsor-marker`。修法 (2)：`hideInsideArticleByKeyword` 加 h1-wrapper guard——含 h1 的 wrapper 即便 class 命中 NOISE_KEYWORD_RE 也不 hide，預防未來 promote 成功後主文標題 wrapper（`.share-header` / `.social-post-header` / `.author-header` 類）被連帶 hide。Fixture `ebc-inline-marker-ad.html` 驗 marker + 字面 regex forcing；`h1-wrapper-header-keyword-guard.html` 驗 guard 保留 header wrapper + 內部 share_bar 仍 hide，sanity check 拿掉 guard 兩條 assertion fail。bug (A) h1 漏 scope 的真實根因是 ebc 深層 single-child wrapper 結構（promote 3 hops 不夠、4 hops 又 scope 擴大吃進其他 sibling chrome），需 promote+narrow 聯動機制架構升級，暫記 `test/PENDING_REGRESSION.md`。bbc 廣告占位殘留 Playwright 環境重現不到（廣告 JS 未注入），亦入 PENDING_REGRESSION。160 spec 全過。

`0.7.7`（v0.7.5 regression 修法：ambiguous confidence penalty 改「從 top-5 挑 POSITIVE 命中者」）。Jimmy 2026-04-23 回報 upmedia.mg /tw/focus/comprehensive/256956 從 v0.7.5 起無法偵測主文。Probe 擷取根因：真實頁面 heuristic top1 是無語意 wrapper DIV（raw 7、score 10.26、無 POSITIVE）、top2 是 `.news-box-text` 真主文（raw 3.5、score 10.17、POSITIVE 命中）；top1/top2 比值 1.009 < 1.25 觸發 ambiguous，v0.7.5 的 `confidence × 0.85` 把 confidence 從 0.3026 壓到 0.2572 < MIN_CONFIDENCE 0.30 → detector 回 null → 整個網頁無法進閱讀模式。修法：**回滾 v0.7.5 的 confidence penalty、改動「選哪個」而非「打折 confidence」**——`detectByHeuristic` 在 ambiguous 分支從 top-5 裡優先挑 POSITIVE 命中且 NEGATIVE 沒命中的候選，找不到才 fallback 到 top[0]。貼近 Readability.js `nbTopCandidates` 精神（top-N 裡 class weight 最好者勝出）。candidates 加 `posHit` / `negHit` flag。保留 `result.ambiguous` 作為 promote hops 收緊信號（upmedia v0.7.2 坑的一般化防守仍在）。Fixture `upmedia-heuristic-ambiguous-positive-wins.html` 精確設計 seedScore：wrapper 3 段 each 1 逗號 +100+ 字 raw 9 → score 10.5；news-box-text 3 段 each 0 逗號 +100+ 字 raw 6 × 1.25 POSITIVE → score 9.72；ratio 1.03 < 1.25 ambiguous、舊邏輯 × 0.85 把 0.30 打到 0.255 fail、新邏輯挑 POSITIVE 勝出 pass。**字面 regex forcing**（`top.find(c => c.posHit`）防未來回退；sanity check 回滾 → 5 條 assertion 全 fail。harness 四站全過（upmedia articleFound=true 恢復，line today / chinatimes / udn 無殘留）。154 spec 全過。
