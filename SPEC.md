...
5. **測試覆蓋**：220 jsdom spec（含 42 條 fixture 重現站點 bug）+ 5 e2e spec（SW wire-up 真 Chrome 驗證）+ e2e harness 基礎設施（`tools/e2e-harness.js` + `test/e2e/`，harness 額外內建 gap audit 警告 >= 80px 可疑留白）。
