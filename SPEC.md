...
最新：**v1.5.18**。詳細修法見 [`CHANGELOG.md`](CHANGELOG.md) 頂部條目；`package.json` / `jread/manifest.json` 為真實版本號來源（`test/version-check.spec.js` forcing function 強制四邊同步：manifest / package.json / SPEC / CHANGELOG）。
...
│   │   ├── main.js              # 進入點、事件串接
│   │   └── orion-detect.js      # Orion（Kagi）瀏覽器偵測：world:MAIN 獨立 content_scripts entry（run_at document_start），讀頁面 window.kagi → 替 <html> 蓋 .jread-orion + 設 --jread-orion-top（v1.5.18）
│   ├── popup/
│   │   ├── popup.html
│   │   └── popup.js
│   ├── options/
