...
(function () {
  ...
  // v0.8.165：懸浮按鈕長按選單「送到 Readwise Reader」的 content 端直送流程（Safari /
  // iOS / iPadOS 用）。
  //
  // 動機：原本長按選單送 Readwise 走 CUSTOM_COMMAND → SW sendToReadwiseFromCommand，
  // 由 SW 端 fetch。iOS Safari 的背景頁（SW / event page）被系統積極掛起、背景 fetch
  // 會 silently 失敗——與「快速鍵送出」同一先天限制（Chrome / macOS 正常）。指紋：
  // 使用者點了沒反應、無任何 toast（SW 連結果都回不來）。同樣根因 popup 端早已改成
  // 「extension 頁直接 fetch」（v0.8.65 saveReaderPayload）繞過 SW。
  //
  // 修法：在 content script（前景分頁、不會被掛起）直接跑 extract + fetch。Safari /
  // Firefox 的 content script 帶 host_permissions 的跨來源 fetch 權限（<all_urls>），
  // fetch readwise.io 不受 CORS 擋；Chrome 自 85 起 content fetch 改走頁面來源 CORS
  // 會被擋，故 Chrome 仍走 SW（floating-icon.js sendToReadwise 依 runtime 分流）。
  // 與 SW 軌共用 popup-core 的 buildReadwisePayload / saveToReadwise / readwiseResultToast，
  // 差別只在 fetch 的執行環境（SW vs content），不雙實作送出邏輯與結果訊息。
  async function sendCurrentPageToReadwise() {
    const P = (typeof window !== 'undefined' && window.__JReadPopup) || null;
    if (!P) { showToast('送出失敗：模組未載入', 'error'); return; }

    // 1. 確認閱讀模式啟動；未啟動先進入（await 到 cleaner / styler 跑完即可 extract，
    //    不需 SW 軌那種 800ms 猜測 buffer——enterReaderMode 是 async、回傳即就緒）。
    if (!NS.state.active && !NS.state.cinemaActive) {
      const ok = await enterReaderMode();
      if (!ok) { /* enterReaderMode 失敗時已自行彈「無法偵測主文」toast */ return; }
    }

    // 2. 抽 reader card payload（content 端直接呼叫，不經訊息往返）
    const extracted = extractReaderPayload();
    if (!extracted || !extracted.ok) {
      showToast(extracted && extracted.reason === 'NOT_APPLICABLE_IN_CINEMA'
        ? '影院模式無法送到 Readwise' : '閱讀模式未啟動', 'error');
      return;
    }
    ...
    // 4. 選用：Gemini 三句摘要（與 popup / SW 軌一致；失敗 fallback 不帶 summary 照送）
    const p = extracted.payload || {};
    if (readwiseSummary && geminiApiKey && p.text) {
      try {
        const sum = await P.generateGeminiSummary({
    ...
    // 5. 送出（content script fetch，Safari host_permissions CORS 豁免）+ 結果 toast
    let result;
    try {
      result = await P.saveReaderPayload({ payload: p, getToken: () => Promise.resolve(readwiseToken || '') });
    } catch (_e) {
      result = { ok: false, error: 'NETWORK' };
    }
    const { message, kind } = P.readwiseResultToast(result);
    showToast(message, kind);
  }
  NS.sendCurrentPageToReadwise = sendCurrentPageToReadwise;
