...
**v0.8.33** — fix(WPA 二次修正：keepalive 回歸 + wake ping): v0.8.32 onStartup 在 WPA 實測仍無效。同 session A/B 行為實測（Jimmy：⌥S 翻譯成功 / ⌥R 無反應 / **⌃R 也無反應**）+ WPA 設定面板 AX 實查（youtube.com=允許、⌥R/⌥⇧R/⌥Y 已正確註冊、menu item enabled）翻出兩個被疊在一起的問題：**(1) v0.8.30 的 keep-alive port 自己是 WPA 回歸根因**——⌃R（content 軌、不碰 background）在 v0.8.29 可用、v0.8.30 起死掉；嫌疑機制：WPA 內 background 拉不起來時 `runtime.connect` 回傳值沒有 null guard，`port.onMessage.addListener` TypeError 疑似中止同批 content script 後續 15 個檔案（custom-shortcuts.js / main.js 全沒跑 → JRead 在 WPA 全滅）。修法：port null guard + listener 掛載包 try/catch + **keep-alive port gate 收緊到 `navigator.maxTouchPoints > 0`**（真 iOS / iPadOS 才開；macOS Safari / WPA 不開——macOS background 生命週期正常、本來就不需要 keep-alive，iOS 對策不受影響）。**(2) Shinkansen commands 在 WPA 可用的疑似真差異 = 它的 content 每頁載入會 sendMessage 給 background**（sticky query / log），這發訊息把 WPA 內從不自行啟動的 background 拉起來；JRead 自 v0.7.235 起 content 零訊息。修法：keepalive.js 補 **wake ping**——Safari 全平台 content 載入時發一發 `BG_WAKE_PING`（fire-and-forget、callback 讀 lastError 防 console 警告；Chrome 不發），SW 新增 case 回 `{ok:true}`。safari-keepalive.spec 重寫成 (A) wake ping gate 矩陣 /(B) port 觸控 gate + null guard forcing /(C) 兩側 wire-up（含 BG_WAKE_PING case）。驗收：TestFlight 更新後重開 YouTube WPA，先測 ⌃R（content 軌復活 = 回歸修復），再測 ⌥R / menu（wake ping 若成功拉起 background 即復活）。
