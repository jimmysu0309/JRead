...
**v0.7.211** — fix: 巴哈姆特哈啦板（forum.gamer.com.tw）reader mode 後段內文圖不載入（只有前幾張顯示）。根因：巴哈姆特用**真實 URL 的 spacer gif**（`https://i2.bahamut.com.tw/none.gif`）當 lazy-load placeholder，below-fold 圖 `src=none.gif` + `data-src=真圖`。`hydrateLazyImages` 的 placeholder 判定 `LAZY_PLACEHOLDER_RE` 只認得空字串 / about:blank / data:image，不認得 none.gif（合法 http URL）→ 把這些圖當「已是真圖」跳過、`data-src` 真圖永遠補不上 → reader mode 下整片空白。實機 vs Playwright 差異（headless 下 gamer lazysizes 未把 src 設成 none.gif、停在空 src 反而被認得）使此 bug 只在實機 Chrome 顯現，故以 harness 強制注入 spacer 狀態（src=none.gif + data-src=真圖）確定性重現根因。修法（cleaner）：新增 `SPACER_SRC_RE` 偵測 spacer-image 慣用檔名（none/blank/spacer/pixel/transparent/grey/loading/placeholder/1x1/dummy/empty… `.gif/png/svg/webp`），這類 src 視同 placeholder，配合真實 `data-src` 一併 hydrate；候選 URL 與 spacer URL 本身也排除這些檔名、`newSrc === prevSrc` 時不改寫。通則屬性：描述 spacer-image 檔名結構特徵，非站點特判。lazy-image-hydration fixture/spec 加 case 6（none.gif spacer + data-src round-trip）。
